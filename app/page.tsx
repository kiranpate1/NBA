...
export default function Home() {
  ...
  const smoothScrollLastTimestampRef = useRef<number | null>(null);
  const smoothScrollVelocityRef = useRef<number>(0);
  ...
  const stopSmoothScroll = useCallback(() => {
    ...
    smoothScrollLastTimestampRef.current = null;
  ...
  const startSmoothScroll = useCallback(() => {
    ...
    const gameRanges = gameRefs
      .map((ref, index) => {
        ...
        return { index, start, end, distance: rect.height };
      })
      .filter(
        (
          entry,
    ...
    if (maxScroll <= 0 || gameRanges.length === 0) return;

    // Requirement: game 1 section distance should take exactly 4 seconds.
    const game1Distance = Math.max(1, gameRanges[0].distance);
    const game1DurationMs = 3000;
    const game1AvgSpeed = game1Distance / game1DurationMs; // px per ms
    const game1StartSpeed = game1AvgSpeed * 0.8;
    const postGame2Speed = game1AvgSpeed * 0.22;
    const slowdownPerGame = 0.78;
    const game1StartY = gameRanges[0].start;
    const game1EndY = gameRanges[0].end;

    const clickStartY = Math.max(0, window.scrollY);
    const preGameDistance = Math.max(0, game1StartY - clickStartY);
    const preGameDurationMs =
      preGameDistance > 0
        ? Math.min(
            2200,
            Math.max(
              900,
              preGameDistance / Math.max(0.0001, game1AvgSpeed * 0.55),
            ),
          )
        : 0;
    ...
    // Monotonic-ish Hermite position curve from 0..1 with tunable start/end slopes.
    const hermitePos = (tRaw: number, m0: number, m1: number) => {
      const t = clamp01(tRaw);
      const h00 = 2 * t * t * t - 3 * t * t + 1;
      const h10 = t * t * t - 2 * t * t + t;
      const h01 = -2 * t * t * t + 3 * t * t;
      const h11 = t * t * t - t * t;
      return h00 * 0 + h10 * m0 + h01 * 1 + h11 * m1;
    };

    const game1M0 = clamp(
      (game1StartSpeed * game1DurationMs) / game1Distance,
      0.2,
      2.2,
    );
    const game1M1 = clamp(
      (postGame2Speed * game1DurationMs) / game1Distance,
      0.1,
      1.6,
    );

    const game1InitialProgress = clamp01(
      (Math.max(clickStartY, game1StartY) - game1StartY) /
        Math.max(1, game1Distance),
    );
    const game1RemainingDurationMs =
      game1DurationMs * (1 - game1InitialProgress);
    const game1CurveStart = hermitePos(game1InitialProgress, game1M0, game1M1);

    const preGameM1 =
      preGameDistance > 0
        ? clamp(
            (game1StartSpeed * preGameDurationMs) / preGameDistance,
            0.1,
            2.5,
          )
        : 0;

    const getTargetSpeed = (scrollY: number) => {
      const range = gameRanges.find(
        (item) => scrollY >= item.start && scrollY < item.end,
      );
      if (!range) {
        const lastIndex = gameRanges.length - 1;
        return (
          postGame2Speed * Math.pow(slowdownPerGame, Math.max(0, lastIndex - 1))
        );
      }

      if (range.index === 0) {
        return postGame2Speed;
      }

      return postGame2Speed * Math.pow(slowdownPerGame, range.index - 1);
    };

    let phase: "pregame" | "game1" | "postgame" =
      clickStartY < game1StartY && preGameDurationMs > 0 ? "pregame" : "game1";
    if (clickStartY >= game1EndY) phase = "postgame";
    let preGameStartTs: number | null = null;
    let game1StartTs: number | null = null;
    let game2EntryTs: number | null = null;
    let hasLockedFastAfterGame2 = false;
    const game2BoostDelayMs = 0;
    const game2BoostSpeed = game1AvgSpeed * 0.9;
    const game2Range = gameRanges.find((r) => r.index === 1) ?? null;

    smoothScrollVelocityRef.current =
      phase === "postgame" ? getTargetSpeed(clickStartY) : 0;

    const tick = (timestamp: number) => {
      if (smoothScrollLastTimestampRef.current === null) {
        smoothScrollLastTimestampRef.current = timestamp;
      }

      const prevY = window.scrollY;
      const deltaMs = Math.max(
        0,
        timestamp - smoothScrollLastTimestampRef.current,
      );
      smoothScrollLastTimestampRef.current = timestamp;

      if (phase === "pregame") {
        if (preGameStartTs === null) preGameStartTs = timestamp;
        const elapsed = Math.max(0, timestamp - preGameStartTs);
        const p = clamp01(elapsed / Math.max(1, preGameDurationMs));
        const eased = hermitePos(p, 0, preGameM1);
        const nextY = clickStartY + (game1StartY - clickStartY) * eased;
        window.scrollTo({ top: Math.min(maxScroll, nextY), behavior: "auto" });

        if (p >= 1 || nextY >= game1StartY - 0.5) {
          phase = "game1";
          game1StartTs = timestamp;
        } else {
          smoothScrollRafRef.current = window.requestAnimationFrame(tick);
          return;
        }
      }

      if (phase === "game1") {
        if (game1StartTs === null) game1StartTs = timestamp;
        const elapsed = Math.max(0, timestamp - game1StartTs);
        const p01 = clamp01(elapsed / Math.max(1, game1RemainingDurationMs));
        const game1Progress =
          game1InitialProgress + (1 - game1InitialProgress) * p01;
        const curveValue = hermitePos(game1Progress, game1M0, game1M1);
        const remappedCurve =
          game1CurveStart < 1
            ? clamp01((curveValue - game1CurveStart) / (1 - game1CurveStart))
            : 1;
        const nextY = game1StartY + game1Distance * remappedCurve;

        window.scrollTo({ top: Math.min(maxScroll, nextY), behavior: "auto" });

        if (p01 >= 1 || nextY >= game1EndY - 0.5) {
          phase = "postgame";
          smoothScrollVelocityRef.current = postGame2Speed;
        } else {
      ...
      // Track when we first enter game 2 and boost speed after the delay.
      if (game2Range && prevY >= game2Range.start && prevY < game2Range.end) {
        if (game2EntryTs === null) game2EntryTs = timestamp;
      } else if (
        game2EntryTs !== null &&
        game2Range &&
        prevY >= game2Range.end
      ) {
        // Already past game 2, keep entry time so it doesn't reset.
      } else if (game2Range && prevY < game2Range.start) {
        game2EntryTs = null;
      }
      ...
      const inGame2Boost =
        game2EntryTs !== null &&
        game2Range !== null &&
        prevY >= game2Range.start &&
        prevY < game2Range.end &&
        timestamp - game2EntryTs >= game2BoostDelayMs;

      if (inGame2Boost) {
        hasLockedFastAfterGame2 = true;
      }

      const targetSpeed = hasLockedFastAfterGame2
        ? game2BoostSpeed
        : getTargetSpeed(prevY);
      const speedLerp = 0.08;
      smoothScrollVelocityRef.current +=
        (targetSpeed - smoothScrollVelocityRef.current) * speedLerp;

      const nextY = Math.min(
        maxScroll,
        prevY + smoothScrollVelocityRef.current * deltaMs,
      );
  ...
  useEffect(() => {
    const startOnFirstClick = () => {
      if (hasAutoScrollStartedRef.current) return;
      hasAutoScrollStartedRef.current = true;
      startSmoothScroll();
    };

    window.addEventListener("pointerdown", startOnFirstClick, {
      passive: true,
      once: true,
    });
  ...
  useEffect(() => {
    ...
    smoothWindow.startSmoothScroll = startSmoothScroll;
    smoothWindow.stopSmoothScroll = stopSmoothScroll;

    return () => {
      stopSmoothScroll();
      delete smoothWindow.startSmoothScroll;
      delete smoothWindow.stopSmoothScroll;
    };
